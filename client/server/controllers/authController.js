  ...
  try {
    ...
    if (error.code === 8 || error.message.includes('RESOURCE_EXHAUSTED') || error.message.includes('Quota exceeded')) {
      ...
      return res.status(503).json({ 
        message: 'Service temporarily unavailable due to high usage. Please try again later.',
  ...
  try {
    ...
    if (req.user && req.user.id) {
      try {
        ...
        console.log(`✅ Logout action for user ${req.user.username} (${req.user.id}) logged successfully`);
      } catch (logError) {
