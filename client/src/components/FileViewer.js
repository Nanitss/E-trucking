...
const FileViewer = ({ documents, truckPlate }) => {
  ...
  const handleFileDownload = (docType, document) => {
    if (document && (document.fullPath || document.filename)) {
      try {
        ...
        // Client documents
        else if (docTypeLower.includes('permit') || docTypeLower === 'business-permits') {
  ...
  const handleOpenFileLocation = (document) => {
    if (document && document.fullPath) {
      try {
        ...
        const folderPath = document.fullPath.substring(0, document.fullPath.lastIndexOf('\\'));
  ...
  return (
    <div className="file-viewer">
      <h4>📁 Truck Documents - {truckPlate}</h4>
          ...
          return (
            <div key={docType} className="document-card">
              ...
              <div className="document-info">
                <h5>{getDocumentName(docType)}</h5>
                <p className="file-name">
                  {getFileTypeIcon(document.filename)} {document.filename}
                </p>
                <p className="file-size">📏 {formatFileSize(document.fileSize)}</p>
                <p className="upload-date">
                  📅 {new Date(document.uploadDate).toLocaleDateString()}
                </p>
                <p className="file-path">📁 {document.relativePath}</p>
              </div>
              <div className="document-actions">
                <button 
                  className="btn-view"
                  onClick={() => handleFileView(docType, document)}
                ...
                <button 
                  ...
                  onClick={() => handleOpenFileLocation(document)}
        ...
        <div className="file-modal-overlay" onClick={closeFileModal}>
          <div className="file-modal" onClick={(e) => e.stopPropagation()}>
            <div className="file-modal-header">
              ...
              <button className="modal-close" onClick={closeFileModal}>✕</button>
            </div>
                ...
                {isImageFile(selectedDoc.document.filename) ? (
                  <div className="image-preview">
                    ...
                    <div className="image-fallback" style={{ display: 'none' }}>
                      ...
                      <p className="fallback-text">File: {selectedDoc.document.filename}</p>
                    </div>
                  </div>
                ) : (
                  <div className="file-icon-preview">
                    ...
                    <p className="file-type-text">{selectedDoc.document.filename.split('.').pop()?.toUpperCase()} File</p>
                  </div>
                )}
              </div>
              ...
              <div className="file-details">
                <div className="detail-section">
                  ...
                  <div className="detail-grid">
                    <div className="detail-item">
                      ...
                      <span className="detail-value">{getDocumentName(selectedDoc.docType)}</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">File Name:</span>
                      <span className="detail-value">{selectedDoc.document.filename}</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">File Size:</span>
                      <span className="detail-value">{formatFileSize(selectedDoc.document.fileSize)}</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Upload Date:</span>
                      <span className="detail-value">
                        {new Date(selectedDoc.document.uploadDate).toLocaleString()}
                      </span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Last Modified:</span>
                      <span className="detail-value">
                        {new Date(selectedDoc.document.lastModified).toLocaleString()}
                      </span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">MIME Type:</span>
                      <span className="detail-value">{selectedDoc.document.mimeType}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="detail-section">
                  ...
                  <div className="file-path-display">
                    ...
                    <p className="path-value">{selectedDoc.document.fullPath}</p>
                    <p className="path-label">Relative Path:</p>
                    <p className="path-value">{selectedDoc.document.relativePath}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="file-modal-footer">
              <button 
                ...
                onClick={() => handleFileDownload(selectedDoc.docType, selectedDoc.document)}
              >
                📄 View Document
              </button>
              <button 
