...
const LocationPicker = ({ onSelectLocation, initialAddress = '', locationType = 'pickup' }) => {
  // Essential refs
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (googleObjectsRef.current.map) {
        try {
          window.google.maps.event.clearInstanceListeners(googleObjectsRef.current.map);
        } catch (e) {
      ...
      if (googleObjectsRef.current.marker) {
        try {
          ...
          window.google.maps.event.clearInstanceListeners(googleObjectsRef.current.marker);
        } catch (e) {
      ...
      if (googleObjectsRef.current.autocomplete) {
        try {
          window.google.maps.event.clearInstanceListeners(googleObjectsRef.current.autocomplete);
        } catch (e) {
  ...
  const handlePlaceSelection = useCallback((place, map, marker) => {
    if (!map || !marker) {
  ...
  const updateAddressFromLatLng = useCallback((latLng) => {
  ...
  const handleSearchResultClick = useCallback((placeId) => {
    setShowSearchResults(false);
    
    if (!window.google?.maps?.places || !googleObjectsRef.current.map) return;
    
    try {
      const placesService = new window.google.maps.places.PlacesService(googleObjectsRef.current.map);
