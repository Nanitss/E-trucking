...
const PinnedLocations = () => {
  ...
  // Location categories
  const categories = [
    { value: 'business', label: 'Business', icon: FaBuilding, color: '#3b82f6' },
    { value: 'residential', label: 'Residential', icon: FaHome, color: '#10b981' },
    { value: 'commercial', label: 'Commercial', icon: FaStore, color: '#f59e0b' },
    { value: 'industrial', label: 'Industrial', icon: FaIndustry, color: '#ef4444' }
  ];
  ...
  const loadPinnedLocations = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5007/api/client/pinned-locations', {
        headers: {
      ...
      console.error('Error loading pinned locations:', error);
              showError('Failed to load saved locations', 'Unable to retrieve your saved locations. Please try again.');
      setLocations([]);
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        showSuccess(
          ...
          showEditModal ? 'Your location has been updated successfully!' : 'Your location has been saved successfully!'
        );
  ...
  const handleDeleteLocation = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5007/api/client/pinned-locations/${selectedLocation.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      if (response.ok) {
        showSuccess('Location Deleted', 'Your location has been deleted successfully!');
        await loadPinnedLocations();
      ...
      showError('Delete Failed', 'Unable to delete location. Please try again.');
    }
  };
  ...
  if (loading) {
    return (
      <div className="pinned-locations-loading">
  ...
  return (
    <div className="pinned-locations">
      {/* Header */}
      <div className="pinned-locations-header">
        <div className="header-content">
          ...
          <div className="header-actions">
            <button className="btn-primary add-location-btn" onClick={openAddModal}>
              <FaPlus /> Add Location
            </button>
          </div>
        </div>
                    ...
                    <tr key={location.id} className="location-row">
                      <td>
                        <div className="location-name-cell">
                          ...
                          {location.isDefault && (
                            <FaStar className="default-star" title="Default location" />
                          )}
                          {location.notes && (
                      ...
                      <td>
                        {location.contactPerson ? (
                          <div className="contact-cell">
                            <div className="contact-person">{location.contactPerson}</div>
              ...
              <div className="form-grid">
                ...
                <div className="form-group full-width">
                  <label className="checkbox-label">
                    ...
                    />
                    <span className="checkbox-text">Set as default location</span>
                  </label>
                </div>
              </div>
            </div>
