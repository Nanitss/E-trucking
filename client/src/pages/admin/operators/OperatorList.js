...
const OperatorList = () => {
  ...
  const filteredOperators = useMemo(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(operator =>
        ...
        operator.OperatorUserName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const getSummaryData = () => {
    ...
    const active = operators.filter(o => o.OperatorStatus?.toLowerCase() === 'active').length;
    const inactive = operators.filter(o => o.OperatorStatus?.toLowerCase() === 'inactive').length;
    const pending = operators.filter(o => o.OperatorStatus?.toLowerCase() === 'pending').length;
  ...
  const uniqueStatuses = [...new Set(operators.map(o => o.OperatorStatus).filter(Boolean))];
  ...
  const handleDelete = async (operatorId) => {
    if (window.confirm('Are you sure you want to delete this operator? This action cannot be undone.')) {
      try {
        ...
        setOperators(operators.filter(operator => operator.OperatorID !== operatorId));
      } catch (err) {
    ...
    <div className="admin-page-container">
      ...
      <div className="admin-content">
        ...
        <div className="greeting-section">
          ...
          <p className="greeting-subtitle">Manage operator accounts and status</p>
          ...
          <div className="summary-cards">
            ...
            <div className="summary-card">
              ...
              <div className="card-content">
                <div className="card-value">{operators.filter(o => o.status === 'active').length}</div>
            ...
            <div className="summary-card">
              ...
              <div className="card-content">
                <div className="card-value">{operators.filter(o => o.status === 'inactive').length}</div>
            ...
            <div className="summary-card">
              ...
              <div className="card-content">
                <div className="card-value">{operators.filter(o => o.role === 'dispatcher').length}</div>
        ...
        <div className="no-data">
          ...
          {!error && !searchTerm && statusFilter === 'all' && (
            <div style={{ marginTop: '10px', fontSize: '12px', color: '#666' }}>
              If you believe this is an error, check console for API debugging information.
            </div>
          )}
        </div>
      ) : (
                ...
                <tr key={operator.OperatorID}>
                  ...
                  <td>
                    {operator.OperatorEmploymentDate
                      ? new Date(operator.OperatorEmploymentDate).toLocaleDateString()
                      : '-'}
                  </td>
                  <td>
                    <span className={`status-badge ${getStatusBadgeClass(operator.OperatorStatus)}`}>
