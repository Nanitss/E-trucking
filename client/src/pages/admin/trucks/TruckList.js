...
const TruckList = ({ currentUser }) => {
  ...
  const handleRecalculateKm = async (truckId, truckPlate) => {
    try {
      ...
      const [fetchResponse, deliveriesResponse, allocationsResponse] = await Promise.all([
      ...
      // Update allocations
      const allocationsData = Array.isArray(allocationsResponse.data) ? allocationsResponse.data : [];
      setAllocations(allocationsData);
       
      // Handle both array response and object with trucks property
      const trucksData = Array.isArray(fetchResponse.data) ? fetchResponse.data : fetchResponse.data.trucks;
      ...
      if (trucksData && trucksData.length > 0) {
        const formattedTrucks = trucksData.map(truck => {
          ...
          const hasActiveDelivery = deliveriesData.some(delivery => {
            ...
            return deliveryTruckId === tId && (deliveryStatus === 'pending' || deliveryStatus === 'in-progress');
          });
          ...
          return {
            ...
            registrationExpiryDaysRemaining: truck.registrationExpiryDaysRemaining || null,
            TruckStatus: truck.truckStatus || truck.TruckStatus || 'unknown',
            TotalKilometers: truck.TotalKilometers || truck.totalKilometers || 0,
            TotalDeliveries: truck.TotalDeliveries || truck.totalCompletedDeliveries || 0,
            ...
            StatusSummary: hasActiveDelivery ? 'On Delivery' : (hasActiveAllocation ? 'Allocated' : (truck.statusSummary || 'Available')),
            IsAvailable: !hasActiveDelivery && !hasActiveAllocation && (truck.isAvailable !== false),
  ...
  const isTruckInActiveDelivery = (truckId) => {
    return deliveries.some(delivery => {
      const deliveryTruckId = delivery.TruckID || delivery.truckId;
      const deliveryStatus = (delivery.DeliveryStatus || delivery.deliveryStatus || '').toLowerCase();
      return deliveryTruckId === truckId && (deliveryStatus === 'pending' || deliveryStatus === 'in-progress');
    });
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [trucksResponse, deliveriesResponse, allocationsResponse] = await Promise.all([
          axios.get(`${baseURL}/api/trucks/actual-documents`),
          axios.get(`${baseURL}/api/deliveries`).catch(err => {
            console.warn('Failed to fetch deliveries:', err);
            return { data: [] };
        ...
        console.log('Raw API response (trucks with actual documents):', trucksResponse.data);
        console.log('Raw API response (deliveries):', deliveriesResponse.data);
        console.log('Raw API response (allocations):', allocationsResponse.data);
      
        // Store deliveries
        const deliveriesData = Array.isArray(deliveriesResponse.data) ? deliveriesResponse.data : [];
        setDeliveries(deliveriesData);
        console.log(`📦 Loaded ${deliveriesData.length} deliveries for truck status validation`);
        
        // Store allocations
        const allocationsData = Array.isArray(allocationsResponse.data) ? allocationsResponse.data : [];
        setAllocations(allocationsData);
        console.log(`🔗 Loaded ${allocationsData.length} allocations for truck status validation`);
        
        // Handle both array response and object with trucks property
        const trucksData = Array.isArray(trucksResponse.data) ? trucksResponse.data : trucksResponse.data.trucks;
        ...
        if (trucksData && trucksData.length > 0) {
          ...
          const validTrucks = trucksData.filter(truck => {
            ...
            if (!truckId || truckId.length !== 20 || /document/i.test(truckId)) {
              console.warn(`⚠️ Skipping truck with invalid ID: ${truckId}`, truck);
              return false;
            }
            return true;
          });
          
          console.log(`📊 Filtered ${trucksData.length - validTrucks.length} invalid trucks`);
          ...
          const formattedTrucks = validTrucks.map(truck => {
            ...
            const hasActiveDelivery = deliveriesData.some(delivery => {
              const deliveryTruckId = delivery.TruckID || delivery.truckId;
              const deliveryStatus = (delivery.DeliveryStatus || delivery.deliveryStatus || '').toLowerCase();
              const isMatch = deliveryTruckId === truckId && (deliveryStatus === 'pending' || deliveryStatus === 'in-progress');
              
              if (isMatch) {
                console.log(`🚛 Truck ${truck.truckPlate || truckId} has active delivery (${deliveryStatus})`);
              }
            ...
            const hasActiveAllocation = allocationsData.some(allocation => {
              const allocationTruckId = allocation.truckId || allocation.TruckID;
              const allocationStatus = (allocation.status || '').toLowerCase();
              const isMatch = allocationTruckId === truckId && allocationStatus === 'active';
              
              if (isMatch) {
                console.log(`🔗 Truck ${truck.truckPlate || truckId} has active allocation (Client: ${allocation.clientId || 'Unknown'})`);
              }
              
              return isMatch;
            });
            
            console.log(`Processing truck ${truck.truckPlate || truckId}: hasActiveDelivery=${hasActiveDelivery}, hasActiveAllocation=${hasActiveAllocation}`);
          ...
          console.log('Trucks with documents and delivery status:', formattedTrucks);
          setTrucks(formattedTrucks);
        } else {
  ...
  const statusCounts = {
    total: trucks.length,
    available: trucks.filter(truck => truck.OperationalStatus === 'active').length, // All active trucks
    ...
    maintenance: trucks.filter(truck => truck.NeedsMaintenance === true || truck.OperationalStatus === 'maintenance').length
  };
  ...
  useEffect(() => {
    ...
    if (complianceFilter !== 'all') {
      filtered = filtered.filter(truck => {
        console.log('Filtering truck by compliance:', truck, 'filter:', complianceFilter);
        if (!truck || !truck.documentCompliance) return complianceFilter === 'incomplete';
        if (!truck.documentCompliance.overallStatus) return complianceFilter === 'incomplete';
  ...
  }, [trucks, statusFilter, allocationFilter, operationalFilter, typeFilter, brandFilter, complianceFilter, searchQuery, isWithinTimeframe]);
    ...
    <div className="admin-page-container">
      ...
      <div className="admin-content">
        ...
        <div className="greeting-section">
          ...
          <p className="greeting-subtitle">Manage your fleet vehicles, track documents, and monitor compliance status</p>
          ...
          <div className="summary-cards">
            ...
            <div className="summary-card">
              ...
              <div className="card-content">
                <div className="card-value">{trucks.filter(t => t.OperationalStatus === 'active').length}</div>
            ...
            <div className="summary-card">
              ...
              <div className="card-content">
                <div className="card-value">{trucks.filter(t => t.IsAllocated === true).length}</div>
            ...
            <div className="summary-card">
              ...
              <div className="card-content">
                <div className="card-value">{trucks.filter(t => t.NeedsMaintenance === true || t.OperationalStatus === 'maintenance').length}</div>
              ...
              <thead>
                <tr>
                  <th onClick={() => handleSort('TruckPlate')} className="sortable">
                    Plate Number {sortField === 'TruckPlate' && (sortDirection === 'asc' ? '↑' : '↓')}
                  </th>
                  <th onClick={() => handleSort('TruckType')} className="sortable">
                    Type {sortField === 'TruckType' && (sortDirection === 'asc' ? '↑' : '↓')}
                  </th>
                  <th onClick={() => handleSort('TruckBrand')} className="sortable">
                    Brand {sortField === 'TruckBrand' && (sortDirection === 'asc' ? '↑' : '↓')}
                  </th>
                  <th onClick={() => handleSort('ModelYear')} className="sortable">
                    Year {sortField === 'ModelYear' && (sortDirection === 'asc' ? '↑' : '↓')}
                  </th>
                  <th onClick={() => handleSort('OperationalStatus')} className="sortable">
                    Status {sortField === 'OperationalStatus' && (sortDirection === 'asc' ? '↑' : '↓')}
                  </th>
                  ...
                  <tr key={truck.TruckID} className="truck-row">
                    <td className="truck-name-cell">
                      <div className="truck-name-wrapper">
                        ...
                        <span className="truck-name-text">{truck.TruckPlate}</span>
                      </div>
                    </td>
                    <td>
                      <span className="type-badge">{truck.TruckType || 'N/A'}</span>
                    </td>
                    <td>{truck.TruckBrand || 'Unknown'}</td>
                    <td>{truck.ModelYear || 'N/A'}</td>
                    <td>
                      <div className="status-cell">
                        <span className={`status-badge ${getStatusBadgeClass(truck.OperationalStatus)}`}>
                          {formatStatus(truck.OperationalStatus) || 'Active'}
                        </span>
                        
                        {/* Registration expiry warning badge */}
                        {truck.registrationExpiryWarning && (
                          <div className="registration-warning-badge">
                            {truck.OperationalStatus === 'registration-expired' 
                              ? `❌ Registration Expired - Blocked`
                              : `⚠️ Registration expires in ${truck.registrationExpiryDaysRemaining} days`
                            }
                          </div>
                        )}
                      </div>
                    ...
                    <td>
                      <div className="compliance-cell">
                        <span className="compliance-icon">{getComplianceIcon(truck)}</span>
                        <span className="compliance-text">{truck.documentCompliance?.requiredDocumentCount || 0}/3</span>
                      </div>
                    </td>
                    <td>
          ...
          {totalPages > 1 && (
            <div className="pagination">
              ...
              <span className="pagination-info">
                Page {currentPage} of {totalPages} ({filteredTrucks.length} trucks)
              </span>
              <button
              ...
              <div key={truck.TruckID} className="truck-card">
                ...
                <div className="truck-card-content">
                  ...
                  <div className="truck-card-info">
                    <div className="truck-info-item">
                      ...
                      <span className="truck-info-value">{truck.TotalDeliveries || 0}</span>
                    </div>
                    <div className="truck-info-item">
                   ...
                   <div className="file-counter-badge">
                     ...
                     <div className="file-count-breakdown">
                       <span className="required-count">
                         {truck.documentCompliance?.requiredDocumentCount || 0}/3 required
                       </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="truck-card-actions">
                  <div className="truck-card-actions-left">
                    ...
                    >
                      {recalculatingKm[truck.TruckID] ? <TbClock className="btn-icon" /> : <TbActivity className="btn-icon" />}
                      Recalc KM
                    </button>
                  </div>
                  <button 
            ...
            <div className="modal-body">
              <div className="details-grid">
                ...
                <div className="details-section">
                  ...
                  <div className="details-items">
                    <div className="detail-item">
                      ...
                      <span className="detail-value">{selectedTruck.TruckID}</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Plate Number:</span>
                      <span className="detail-value">{selectedTruck.TruckPlate}</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Type:</span>
                      <span className="detail-value">{selectedTruck.TruckType}</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Brand:</span>
                      <span className="detail-value">{selectedTruck.TruckBrand || 'Unknown'}</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Model Year:</span>
                      <span className="detail-value">{selectedTruck.ModelYear || 'Not specified'}</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Capacity:</span>
                      <span className="detail-value">{selectedTruck.TruckCapacity || 'Not specified'}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="details-section">
                  ...
                  <div className="details-items">
                    <div className="detail-item">
                      ...
                      <span className="detail-value">{selectedTruck.TotalKilometers?.toLocaleString() || 0} km</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Completed Deliveries:</span>
                      <span className="detail-value">{selectedTruck.TotalDeliveries || 0}</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Average KM per Delivery:</span>
                      <span className="detail-value">{selectedTruck.AverageKmPerDelivery || 0} km</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Last Odometer Update:</span>
                      <span className="detail-value">{selectedTruck.LastOdometerUpdate ? new Date(selectedTruck.LastOdometerUpdate).toLocaleDateString() : 'Never'}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="details-section">
                  ...
                  <div className="details-items">
                    <div className="detail-item">
                      ...
                      <span className={`detail-badge ${getStatusBadgeClass(selectedTruck.OperationalStatus)}`}>
                        {formatStatus(selectedTruck.OperationalStatus)}
                      </span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Allocation Status:</span>
                      <span className={`detail-badge ${getStatusBadgeClass(selectedTruck.AllocationStatus)}`}>
                        {formatStatus(selectedTruck.AllocationStatus)}
                      </span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Availability:</span>
                      <span className={`detail-badge ${selectedTruck.IsAvailable ? 'status-available' : 'status-busy'}`}>
                        {selectedTruck.IsAvailable ? 'Available' : 'Not Available'}
                      </span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Currently Allocated:</span>
                      <span className={`detail-badge ${selectedTruck.IsAllocated ? 'status-allocated' : 'status-available'}`}>
                        {selectedTruck.IsAllocated ? 'Yes' : 'No'}
                      </span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">In Use:</span>
                      <span className={`detail-badge ${selectedTruck.IsInUse ? 'status-busy' : 'status-available'}`}>
                        {selectedTruck.IsInUse ? 'Yes' : 'No'}
                      </span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Needs Maintenance:</span>
                      <span className={`detail-badge ${selectedTruck.NeedsMaintenance ? 'status-maintenance' : 'status-available'}`}>
                ...
                <div className="details-section">
                  ...
                  <div className="details-items">
                    {(() => {
                      const requirements = getLicenseRequirements(selectedTruck.TruckType);
                      return (
                        <>
                          <div className="detail-item">
                            <span className="detail-label">Driver License Required:</span>
                            <span className="detail-value">{requirements.driverLicense}</span>
                          </div>
                          <div className="detail-item">
                            <span className="detail-label">Helpers Required:</span>
                            <span className="detail-value">{requirements.helpers}</span>
                          </div>
                          <div className="detail-item">
                            <span className="detail-label">Helper Requirements:</span>
                            <span className="detail-value">{requirements.helperRequirements.join(', ') || 'None'}</span>
                          </div>
                        </>
                      );
                    })()}
                ...
                <div className="details-section">
                  ...
                  <div className="details-items">
                    <div className="detail-item">
                      ...
                      <span className={`detail-badge compliance-${getComplianceClass(selectedTruck).replace('compliance-', '')}`}>
                        {getComplianceStatus(selectedTruck)}
                      </span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Documents Completed:</span>
                      <span className="detail-value">{getComplianceDetails(selectedTruck)}</span>
                    </div>
                    
                    {/* Document Status List */}
                    <div className="documents-list">
                      <div className="document-item">
                        <span className="doc-name">OR (Original Receipt):</span>
                        <span className={`doc-status ${selectedTruck.documents?.orDocument ? 'complete' : 'missing'}`}>
                          {selectedTruck.documents?.orDocument ? '✓ Complete' : '✗ Missing'}
                        </span>
                      </div>
                      <div className="document-item">
                        <span className="doc-name">CR (Certificate of Registration):</span>
                        <span className={`doc-status ${selectedTruck.documents?.crDocument ? 'complete' : 'missing'}`}>
                          {selectedTruck.documents?.crDocument ? '✓ Complete' : '✗ Missing'}
                        </span>
                      </div>
                      <div className="document-item">
                        <span className="doc-name">Insurance:</span>
                        <span className={`doc-status ${selectedTruck.documents?.insuranceDocument ? 'complete' : 'missing'}`}>
                          {selectedTruck.documents?.insuranceDocument ? '✓ Complete' : '✗ Missing'}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="document-summary">
                    <div className="document-count-badge">
                      ...
                      <span className="count-text">
                        {selectedTruck.documentCompliance?.documentCount || 0} files uploaded
                      </span>
                    </div>
                    <div className="document-breakdown">
                      <span className="required-docs">
                        Required: {selectedTruck.documentCompliance?.requiredDocumentCount || 0}/3
                      </span>
                      <span className="optional-docs">
                        Optional: {selectedTruck.documentCompliance?.optionalDocumentCount || 0}/1
                      </span>
                    </div>
                  </div>
                  {selectedTruck.documents && Object.keys(selectedTruck.documents).length > 0 ? (
                    <div className="details-items">
                      <FileViewer 
                  ...
                  <div className="details-items">
                    <div className="detail-item">
                      ...
                      <span className="detail-value">
                        {selectedTruck.DateAdded ? new Date(selectedTruck.DateAdded).toLocaleDateString() : 'Unknown'}
                      </span>
                    </div>
                  </div>
                </div>
  ...
  function getComplianceIcon(truck) {
    try {
      ...
      if (!truck || !truck.documentCompliance || !truck.documentCompliance.overallStatus) return '⚠';
      const { overallStatus } = truck.documentCompliance;
  ...
  function getComplianceDetails(truck) {
    try {
      ...
      if (truck.documentCompliance.documentCount !== undefined) {
        const requiredCount = truck.documentCompliance.requiredDocumentCount || 0;
        const optionalCount = truck.documentCompliance.optionalDocumentCount || 0;
