    ...
    const fetchAuditData = async () => {
      try {
        ...
        if (response.data && response.data.items) {
          ...
          console.log('Actions found:', [...new Set(response.data.items.map(item => item.action))].join(', '));
          ...
          let actions = [...new Set(response.data.items.map(item => item.action))].filter(Boolean);
          ...
          const entityTypes = [...new Set(response.data.items.map(item => item.entityType))].filter(Boolean);
          const users = [...new Set(response.data.items.map(item => item.user))].filter(Boolean);
